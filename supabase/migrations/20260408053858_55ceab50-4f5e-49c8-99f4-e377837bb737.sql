...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      'excel_row_id', coalesce(dl.excel_row_id, '__legacy_' || dense_rank() over (order by dl.employee_id, dl.notes)),
    ...
    GROUP BY coalesce(dl.excel_row_id, '__legacy_' || dl.employee_id::text || '|' || coalesce(dl.notes, ''))
  ) sub;
...
GRANT EXECUTE ON FUNCTION public.get_deployment_rows(uuid) TO authenticated;
